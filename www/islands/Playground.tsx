...
export default function Playground() {
  return (
    <div class={tw`h-full w-full min-h-96 grow flex flex-col rounded-lg bg-gray-50 border-1`}>
      <TopBar />
      <Panels />
    </div>
  );
...
function TopBarDropdown<T extends string[]>(
  ...
  return (
    <div class={tw`relative inline-block text-left`}>
      <div
        class={tw
          `cursor-pointer inline-flex justify-center py-1 px-2 border border-gray-300 font-medium rounded-md text-gray-700 bg-gray-50 transition duration-150 ease-in-out select-none hover:text-gray-500 hover:bg-gray-50 active:bg-gray-100 active:text-gray-700 focus:outline-none focus:shadow focus:border-blue-300`}
      ...
      >
        {unselected.map((option) => (
          <li
            class={tw`cursor-pointer text-gray-700 block w-full text-left px-4 py-2 active:bg-gray-100 active:text-gray-700 hover:text-gray-500 hover:bg-gray-100`}
            key={option}
          >
            {option}
          </li>
